...
# utilizing the ModelSim Foreign Language Interface (FLI).
...
if [ -z "$IN_DOCKER" ]; then
    docker run --rm -it \
        ...
        --entrypoint bash \
        sha256:281750654c0dcb179ca9ecbef324154c58ec97181eb95881116a8eb7ce71e14 \
        -c "/work/test/test_modelsim.sh"
...
gcc -shared -fPIC -m32 -Bsymbolic -o cosim_jtag.so ../cosim_jtag.c
# libc6-i386

# Run the simulation in the background.
# -> Shared library "cosim_jtag.so" is automatically loaded.
vsim -c tb -foreign "cosim_jtag_dummy ./cosim_jtag.so" -do "run -all" &
